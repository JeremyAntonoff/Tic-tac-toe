...
function updateWinArray() {
  for (var i = 0; i < winArray.length; i++) {
    for (var j = 0; j < winArray[j].length; j++) {
      if (
        gameObj[winArray[i][j]] === pChoice ||
        gameObj[winArray[i][j]] === aiChoice
      ) {
...
function makeWinner() {
  for (var i = 0; i < winArray.length; i++) {
    if (winArray[i][0] === aiChoice && winArray[i][1] === aiChoice) {
      if (
        ticArray[winArray[i][2] - 1] !== pChoice &&
        ticArray[winArray[i][2] - 1] !== aiChoice &&
        ticArray[winArray[i][2] - 1] !== undefined
      ) {
        ticArray[winArray[i][2] - 1]
          .text(aiChoice)
          .css('text-transform', 'uppercase');
      ...
      if (
        ticArray[winArray[i][1] - 1] !== pChoice &&
        ticArray[winArray[i][1] - 1] !== aiChoice &&
        ticArray[winArray[i][1] - 1] !== undefined
      ) {
        ticArray[winArray[i][1] - 1]
          .text(aiChoice)
          .css('text-transform', 'uppercase');
      ...
      if (
        ticArray[winArray[i][0] - 1] !== pChoice &&
        ticArray[winArray[i][0] - 1] !== aiChoice &&
        ticArray[winArray[i][0] - 1] !== undefined
      ) {
        ticArray[winArray[i][0] - 1]
          .text(aiChoice)
          .css('text-transform', 'uppercase');
